...
var django_url_watermark = "http://65.108.1.206:8008/django-resize/watermark/";

bot.use(async (ctx) => {
    console.log(ctx.message);
    for (let i of allowed_users) {
        if (i == ctx.chat.id) {
            if (ctx.message.video || (ctx.message.video && ctx.message.caption)) {
            ...
            } else if (ctx.message.photo || (ctx.message.caption && ctx.message.photo)) {
